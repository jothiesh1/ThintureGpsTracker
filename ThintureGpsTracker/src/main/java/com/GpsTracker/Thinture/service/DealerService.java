...
public class DealerService {
    ...
    public Dealer addDealer(Dealer dealer, CustomUserDetails loggedInUser) {
        ...
        logger.info("Adding dealer. Logged-in user: {} (ID: {}), Role: {}", loggedInUser.getUsername(), userId, role);

        // Assign the dealer to the logged-in user's category
        if (role.equals("ROLE_ADMIN")) {
            Admin admin = adminRepository.findById(userId)
                .orElseThrow(() -> new RuntimeException("Admin not found"));
            dealer.setAdmin(admin);
            dealer.setSuperAdmin(null); // Ensure only one of them is set
            logger.info("Dealer assigned to Admin ID: {}", userId);
            ...
            logger.info("Dealer assigned to SuperAdmin ID: {}", userId);
        } else {
