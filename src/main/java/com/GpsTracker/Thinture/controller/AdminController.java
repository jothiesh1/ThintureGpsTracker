...
public class AdminController {
    ...
    public String showAddAdminForm(Model model) {
        ...
        return "add_admin"; // HTML form for adding an admin
    }
    ...
    public ResponseEntity<String> toggleAdminStatus(@PathVariable Long id) {
        Optional<Admin> optionalAdmin = adminService.toggleStatus(id);

        if (optionalAdmin.isPresent()) {
            Admin updated = optionalAdmin.get();
            String statusMessage = updated.isStatus() ? "Unlocked (Active)" : "Locked (Inactive)";
    ...
    @PutMapping("/{id}/edit")
    public ResponseEntity<String> updateAdmin(@PathVariable Long id, @RequestBody Admin updatedData) {
        Optional<Admin> adminOpt = adminRepository.findById(id);
        if (adminOpt.isPresent()) {
            Admin admin = adminOpt.get();
            admin.setCompanyName(updatedData.getCompanyName());
            admin.setEmail(updatedData.getEmail());
            admin.setAddress(updatedData.getAddress());
            admin.setPhone(updatedData.getPhone());
            admin.setCountry(updatedData.getCountry());
